...
export default function ReceiptPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-[#f8fafc] px-4 pt-12 pb-8">
        {/* Top bar */}
        <div className="flex items-center justify-between mb-4 no-print">
          <button onClick={() => router.back()} className="flex items-center gap-1.5 text-gray-500 text-sm">
        ...
        {hasContact && (
          <div className="flex gap-2 mb-4 no-print max-w-md mx-auto">
            {booking.customer_phone && (
              <a
                href={smsHref}
                className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-white border border-gray-200 text-gray-600 text-sm font-medium active:opacity-70"
              >
        ...
        {/* Receipt card */}
        <div className="receipt-card bg-white border border-gray-200 rounded-2xl p-6 max-w-md mx-auto">
          {/* Header */}
          <div className="text-center mb-6 pb-5 border-b border-gray-200 print-divider">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src="/logo.png" alt="EV Exec" width={64} height={64} className="mx-auto mb-3" />
            <p className="text-gray-400 text-[10px] uppercase tracking-widest print-gray">Journey Receipt</p>
          ...
          <div className="flex items-center justify-center gap-2 mb-5">
            ...
            {booking.completed_at && (
              <span className="text-gray-400 text-xs print-label">
          ...
          <div className="bg-gray-50 rounded-xl p-3 mb-5 border border-gray-100">
            {booking.pickup_location && (
              <div className="flex gap-2 mb-2.5">
                ...
                <span className="text-gray-900 text-sm print-black flex-1">{booking.pickup_location}</span>
              </div>
            )}
            {dropoff && (
              <div className="flex gap-2">
                <span className="text-gray-400 text-xs font-bold w-12 pt-0.5 print-gray flex-shrink-0">TO</span>
                <span className="text-gray-900 text-sm print-black flex-1">{dropoff}</span>
              </div>
            )}
            {!booking.pickup_location && !dropoff && (
              <p className="text-gray-400 text-xs">Route not recorded</p>
            )}
          </div>
          ...
          {/* Financials */}
          <div className="mt-4 pt-4 border-t border-gray-200 print-divider space-y-2">
            ...
            {totalCharged > 0 && (
              <div className="flex items-center justify-between pt-2 border-t border-gray-200 print-divider">
                <span className="text-gray-900 font-bold text-sm print-black">Total</span>
                <span className="text-[#d5a538] font-bold text-lg print-black">£{totalCharged.toFixed(2)}</span>
              </div>
            )}
            <div className="flex items-center justify-between">
              <span className="text-gray-400 text-xs print-label">Payment</span>
              <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${p.className}`}>{p.text}</span>
            </div>
          </div>

          {/* Footer */}
          <p className="text-center text-gray-300 text-[10px] mt-6 print-label">
            EV Exec Chauffeur Services · evexec.co.uk
          </p>
        </div>
      </div>
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-start justify-between gap-4">
      <span className="text-gray-400 text-xs print-label flex-shrink-0 pt-0.5">{label}</span>
      <span className="text-gray-900 text-sm font-medium print-black text-right">{value}</span>
    </div>
  )
}
